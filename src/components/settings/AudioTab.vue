...
const builtInMusicPaths = computedAsync(async () => {
    ...
    result.set('Daft Punk - Veridis Quo (Official Audio).wav', await join(resourcesDir, 'music', 'Daft Punk - Veridis Quo (Official Audio).wav'))
    return result;
})
...
<template>
    <div v-focus-section:[tabSectionId] class="tab-content-container">
        <DropDown :title="'Background music'"
            :values="['The Night Swim.wav', 'HOME - Resonance.wav', 'Daft Punk - Veridis Quo (Official Audio).wav', 'file']"
            :displayValues="['Raulinho - The Night Swim', 'HOME - Resonance', 'Daft Punk - Veridis Quo', 'From file...']"
